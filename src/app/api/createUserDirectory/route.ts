...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const baseDir = 'D:\\booksFollowUp\\pdfScanner'; // Local path for testing
    ...
    // Step 2: Test base directory access
    try {
      ...
      log('error', 'Cannot access base directory', { error, baseDir });
      return NextResponse.json(
        { success: false, message: `Cannot access base directory ${baseDir}. Check if it exists and permissions.` },
        { status: 403 }
      );
    }
...
//     //const baseDir = '\\\\10.20.11.33\\booksFollowUp\\pdfScanner';
//     const baseDir = 'localhost:3000\\booksFollowUp\pdfScanner';
//     const userDir = path.join(baseDir, username).replace(/\\/g, '\\');
